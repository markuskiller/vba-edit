...
class ConfigGenApp(ttk.Window):  # type: ignore[misc]
    ...
    def _generate_toml(self) -> str:
        ...
        # Path fields — only emit when non-empty
        for key, widget in [
            ("file", self._file_row),
            ("vba_directory", self._vba_dir_row),
            ("logfile", self._logfile_row),
        ]:
            val = widget.get()
            if val:
        ...
        # Encoding
        enc_mode = self._encoding_mode.get()
        if enc_mode == "detect":
            lines.append("detect_encoding = true")
        elif enc_mode == "custom":
            enc_val = self._encoding_combo.get().strip()
            if enc_val:
                lines.append(f"encoding = {_q(enc_val)}")

        # Header mode
        hdr = self._header_mode.get()
        if hdr == "save_headers":
            lines.append("save_headers = true")
        elif hdr == "in_file_headers":
        ...
        # References section — only if something is set
        refs_file = self._refs_file_row.get()
        active_refs_flags = [k for k, v in self._refs_bool_vars.items() if v.get()]
